...
let currentCharIndex = mistakesCount = typingActive = 0;

function loadRandomParagraph() {
    const randomIndex = Math.floor(Math.random() * paragraphs.length);
    displayText.innerHTML = "";
    paragraphs[randomIndex].split("").forEach(char => {
        let charSpan = `<span>${char}</span>`;
        displayText.innerHTML += charSpan;
    });
...
function handleTyping() {
    let characters = displayText.querySelectorAll("span");
    let typedChar = inputField.value.split("")[currentCharIndex];

    if (currentCharIndex < characters.length - 1 && timeRemaining > 0) {
        if (!typingActive) {
        ...
        if (typedChar == null) {
            ...
            if (characters[currentCharIndex].innerText == typedChar) {
        ...
        let wordsPerMinute = Math.round(((currentCharIndex - mistakesCount) / 5) / (totalTime - timeRemaining) * 60);
        wordsPerMinute = wordsPerMinute < 0 || !wordsPerMinute || wordsPerMinute === Infinity ? 0 : wordsPerMinute;

        wpmDisplay.innerText = wordsPerMinute;
        mistakeDisplay.innerText = mistakesCount;
        cpmDisplay.innerText = currentCharIndex - mistakesCount;
    } else {
...
function restartGame() {
    ...
    currentCharIndex = mistakesCount = typingActive = 0;
    inputField.value = "";
    timerDisplay.innerText = timeRemaining;
